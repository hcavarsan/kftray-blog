...
All components share the same SQLite database for configuration storage, enabling seamless switching between [desktop](/docs/interfaces/desktop), [terminal](/docs/interfaces/terminal), and [MCP](/docs/interfaces/mcp) interfaces.
...
The [MCP server](/docs/interfaces/mcp) implements the MCP Streamable HTTP transport, exposing kftray operations over JSON-RPC 2.0:
