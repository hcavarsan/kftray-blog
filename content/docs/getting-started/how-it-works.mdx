---
title: How It Works
description: How kftray connects to the Kubernetes API, establishes port-forward tunnels, handles reconnections, and routes TCP and UDP traffic to local ports.
---
...
Both the desktop and terminal interfaces share the same SQLite database at `~/.kftray/kftray.db`. The [MCP server](/docs/configuration/mcp-server) also reads from the same database when enabled. This means:
...
| **kftray-mcp** | [MCP server](/docs/configuration/mcp-server), exposes port-forwarding operations to external clients |
